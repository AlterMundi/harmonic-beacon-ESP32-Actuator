...
bool WiFiManager::connect() {
  ...
  WiFi.disconnect();
  delay(100);
  LOG_TRACE("Attempting to connect to WiFi...with SSID/" + station_cfg.ssid +
            "/pass/ " + station_cfg.password + "/");
...
void WiFiManager::scheduleReconnect() {
  ...
  unsigned long delay =
      min((unsigned long)(connection_timeout * pow(1.5, current_retry - 1)),
          300000UL);
...
void WiFiManager::onWiFiEvent(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:
    ...
    if (instance->reconnect_timer > 0) {
      LOG_TRACE(
          "Reconnection already scheduled, ignoring new disconnection event.");
      return;
    }
    ...
    if (instance->current_retry < instance->max_retries) {
      ...
      if (!instance->old_ssid.isEmpty() && !instance->status.is_transitioning) {
        LOG_TRACE("Maximum retries reached. Attempting to connect with "
        ...
        instance->resetState();
        // If the router is down we want to reconnect immediately once it comes
        // back online
        instance->scheduleReconnect();
      }
    }
  ...
  webServer->on("/wifi", HTTP_GET, [this]() {
    ...
    if (scan_requested) {
      ...
      if (result == WIFI_SCAN_RUNNING) {
        ...
        resumeReconnection();
        String json =
            "{\"message\":\"scan failed\",\"error\":" + String(result) + "}";
    ...
    if (result == WIFI_SCAN_RUNNING) {
      ...
      String json =
          "{\"message\":\"failed to start scan\",\"error\":" + String(result) +
          "}";
      webServer->send(503, "application/json", json);
    } else if (result >= 0) {
      // Scan completado inmediatamente (caso raro)
      LOG_TRACE("Scan completed immediately with " + String(result) +
                " networks");
      resumeReconnection();
      sendScanResults(result);
    } else {
      ...
      resumeReconnection();
      String json = "{\"message\":\"unexpected scan result\",\"error\":" +
                    String(result) + "}";
      webServer->send(503, "application/json", json);
    }
  });
...
void WiFiManager::update() {
  ...
  if (validation_timer > 0 && millis() > validation_timer) {
    ...
    if (status.is_transitioning && !online) {
      LOG_ERROR("New credentials validation failed, reverting to previous "
                "configuration...");
  ...
  if (!online && !reconnect_paused && reconnect_timer > 0 &&
      millis() > reconnect_timer) {
...
void WiFiManager::sendScanResults(int networkCount) {
  ...
  if (networkCount < 0) {
    ...
    json +=
        "\"message\":\"scan failed\",\"error\":" + String(networkCount) + "}";
...
String WiFiManager::generateCaptivePortalPage() {
  ...
  html += "<style>";
  html += ":root{--accent:#00e676;--bg-dark:#121212;--bg-panel:#1e1e1e;--text-"
          "main:#e0e0e0;--text-muted:#888;}";
  html += "*{margin:0;padding:0;box-sizing:border-box;}";
  html += "body{font-family:'Segoe UI', Tahoma, Geneva, Verdana, "
          "sans-serif;background:var(--bg-dark);color:var(--text-main);padding:"
          "20px;min-height:100vh;}";
  html +=
      ".container{background:var(--bg-panel);padding:30px;border-radius:12px;"
      "box-shadow:0 4px 6px rgba(0,0,0,0.3);max-width:600px;margin:0 auto;}";
  html += "h1{color:var(--accent);text-align:center;margin-bottom:20px;border-"
          "bottom:2px solid var(--accent);padding-bottom:12px;font-size:28px;}";
  html += "h1::before{content:'🎵 ';}";
  html += ".subtitle{color:var(--text-muted);font-size:14px;text-align:center;"
          "margin-bottom:25px;}";
  html += ".status{padding:15px;margin:20px "
          "0;border-radius:8px;font-weight:bold;border:1px "
          "solid;text-align:center;}";
  html += ".connected{background:#1b4d3e;color:#00e676;border-color:#00e676;}";
  html +=
      ".disconnected{background:#3e1b1b;color:#ff5252;border-color:#ff5252;}";
  html += ".scanning{background:#3e3e1b;color:#ffd740;border-color:#ffd740;}";
  html += "input,select{width:100%;padding:12px;margin:10px "
          "0;background:#2d2d2d;border:2px solid "
          "#444;border-radius:6px;color:white;font-size:14px;transition:border-"
          "color 0.3s;}";
  html += "input:focus,select:focus{outline:none;border-color:var(--accent);}";
  html += "button{background:transparent;color:var(--accent);border:2px solid "
          "var(--accent);padding:12px "
          "20px;border-radius:6px;cursor:pointer;margin:5px;font-size:16px;"
          "font-weight:600;transition:all 0.2s;}";
  html += "button:hover{background:var(--accent);color:#121212;transform:"
          "translateY(-2px);}";
  html += "button:disabled{border-color:#555;color:#555;cursor:not-allowed;"
          "transform:none;background:transparent;}";
  html += ".btn-secondary{border-color:#888;color:#888;}";
  html += ".btn-secondary:hover{background:#888;color:#121212;}";
  html += ".wifi-section{margin:20px "
          "0;padding:20px;background:#252525;border-radius:8px;border-left:4px "
          "solid var(--accent);}";
  html += "h3{color:var(--accent);margin-bottom:15px;font-size:18px;font-"
          "weight:600;}";
  html += ".network{padding:12px;margin:8px 0;border:1px solid "
          "#444;border-radius:8px;cursor:pointer;background:#2d2d2d;transition:"
          "all 0.2s;}";
  html +=
      ".network:hover{border-color:var(--accent);transform:translateX(5px);}";
  "all 0.3s ease;}";
  html += ".network:hover{background:#f0fff4;border-color:var(--altermundi-"
          "green);transform:translateX(5px);}";
  html += ".network.selected{background:var(--altermundi-green);color:white;"
          "border-color:#48b800;}";
  ...
  html += ".signal-excellent{color:#28a745;}.signal-good{color:#ffc107;}."
          "signal-fair{color:#fd7e14;}.signal-weak{color:#dc3545;}";
  html += ".loading{text-align:center;padding:20px;color:var(--gray-medium);}";
  html += ".form-section{margin-top:30px;padding-top:20px;border-top:2px solid "
          "#ddd;}";
  html += "label{display:block;margin-bottom:6px;color:var(--gray-dark);font-"
          "weight:600;font-size:14px;}";
  html += "</style></head><body>";

  html += "<div class='container'>";
  html += "<h1>WiFi Configuration</h1>";
  html += "<div class='subtitle'>AlterMundi - La pata tecnológica de ese otro "
          "mundo posible</div>";
  html += "<div style='text-align:center;margin-bottom:20px;'>";
  html += "<button class='btn-secondary' "
          "onclick=\"window.location.href='/'\">🏠 Inicio</button>";
  html += "<button class='btn-secondary' "
          "onclick=\"window.location.href='/settings'\">⚙️ Configuración "
          "Avanzada</button>";
  html +=
      "<button onclick=\"window.location.href='/data'\">📊 Ver Datos</button>";
  html += "</div>";
  ...
  html += "<button onclick='scanNetworks()' id='scanBtn'>Scan for "
          "Networks</button>";
  html += "<div id='networks' class='loading'>Click \"Scan for Networks\" to "
          "see available WiFi networks</div>";
  html += "</div>";
  ...
  html += "<input type='text' name='ssid' id='ssid' placeholder='Enter WiFi "
          "SSID' value='";
  ...
  html += "networksDiv.innerHTML = '<div class=\"loading\">Scanning for WiFi "
          "networks...</div>';";
  ...
  html += "}";
  html += "} catch (error) {";
  html += "networksDiv.innerHTML = '<div style=\"color:red; padding:20px; "
          "text-align:center;\">Error: ' + error.message + '</div>';";
  html += "}";
  ...
  html += "}";
  html += "} catch (error) {";
  html += "document.getElementById('networks').innerHTML = '<div "
          "style=\"color:red; padding:20px; text-align:center;\">Error: ' + "
          "error.message + '</div>';";
  html += "document.getElementById('scanBtn').disabled = false;";
  html += "document.getElementById('scanBtn').innerHTML = 'Scan for Networks';";
  html += "scanInProgress = false;";
  ...
  html += "networksDiv.innerHTML = '<div style=\"text-align:center; "
          "padding:20px; color:#666;\">No networks found</div>';";
  html += "return;";
  html += "}";
  html += "let html = '';";
  ...
  html += "html += '<div class=\"network\" onclick=\"selectNetwork(\\'' + "
          "escapeHtml(network.ssid) + '\\', ' + isSecure + ')\">';";
  html += "html += '<div class=\"network-name\">' + escapeHtml(network.ssid) + "
          "'<span style=\"font-size:12px;color:' + (isSecure ? '#dc3545' : "
          "'#28a745') + ';\">' + lockIcon + '</span></div>';";
  html += "html += '<div class=\"network-details ' + signalClass + '\">Signal: "
          "' + signalStrength + ' (' + network.rssi + ' dBm)</div>';";
  html += "html += '</div>';";
  html += "});";
  ...
  html += "}";
  html += "document.querySelectorAll('.network').forEach(n => "
